	...
	<body>
		...
		<%--<div class="address mt">
			<div class="wrapper clearfix">
				<a href="${path}/personalCenter" class="fl">首页</a>
				<span>/</span>
				<a href="${path}/personalDetails" class="on">个人信息</a>
			</div>
		</div>--%>
			...
			<div class="wrapper clearfix">
				<div class="zuo fl">
					<h3>
						...
						<p class="clearfix">${sessionScope.user.username}<%--<a href="${path}/logout">[退出登录]</a>--%></p>
					</h3>
					<div>
						<h4>我的交易</h4>
						<ul>
				...
				<div class="you fl">
					<h2>个人信息</h2>
					<div class="gxin">
						<div class="tx"><a href="#"><img class="user-img" src="${path}/<c:if test="${requestScope.userAccountPicture==null}">static/images/userPicture/morentouxiang.jpeg</c:if><c:if test="${requestScope.userAccountPicture!=null}">${requestScope.userAccountPicture.img_url}</c:if>"/><p id="avatar">修改头像</p></a></div>
						<div class="xx">
							<h3 class="clearfix"><strong class="fl">基础资料</strong><%--<a href="#" class="fr" id="edit1">编辑</a>--%></h3>
							<div>用户名：${sessionScope.user.username}</div>
							<div>电话号码：${sessionScope.user.tel_number}</div>
						<%--	<div>性别：女</div>--%>
							<h3>高级设置</h3>
							<!--<div><span class="fl">银行卡</span><a href="#" class="fr">管理</a></div>-->
							<div><span class="fl">修改密码</span><a href="${path}/updatePassWord" class="fr" >修改</a></div>
						</div>
					</div>			
				</div>
			</div>
